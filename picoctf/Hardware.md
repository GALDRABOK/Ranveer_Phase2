...
1.I started with the file firmware.elf command
```
file firmware.elf
firmware.elf: ELF 32-bit LSB executable, Atmel AVR 8-bit, version 1 (SYSV), statically linked, with debug_info, not stripped
```
which told me it was an Atmel AVR 8-bit executable. This confirmed the "Bare-Metal" part of the problem clue

2.My first idea was to find a simple string. I ran avr-strings firmware.elf (for 8-bit text) and found nothing but technical data.
...
```
I looked at the <main> function (the start of the program). I saw these key lines:

At 0x220: ldi r25, 0xA5 — This "loads" the hex value 0xA5 into a register. This looked like a key.

At 0x244: lpm r24, Z — This "loads data from program memory" (part of the file itself).

At 0x24c: eor r30, r11 — TIt's the Exclusive OR (XOR) command! It was XORing the data it just loaded with the key 0xA5 (which had been moved into register r11).

6.This proved the solution: The file's data is XOR-encoded with the single-byte hex key 0xA5.I used CyberChef to perform the final transformation:
![screenshot of cyberchef](./Screenshots/Hardware_Challenge3_cyberchef.jpg)
## Flag:
...
- The avr-nm output was useful for finding the <main> function, which is where I focused my objdump analysis.
